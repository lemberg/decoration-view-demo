...
class CalendarLayout: UICollectionViewLayout, CalendarLayoutDelegate {
    ...
    func didReceivePinchGesture(gesture: UIPinchGestureRecognizer) {
        ...
        if gesture.state == .began {
            ...
            number = Int(gesture.location(in: collectionView).y / (eventHeignt + 8))
        ...
        if gesture.state == .changed {
            ...
            let centerZoomPointY = (contentHeight - collectionViewHeight) / CGFloat(2) // 12
            
            let pointY = ( ((centerZoomPointY / 12) - 8) * CGFloat(number) )
            print("\(pointY) = (\(centerZoomPointY)/12) * \(number)")
            
            gestureOffsetY = pointY
